...
Matching is on a package's **whole** license string, not the parts of an AND/OR expression. A key of `"MPL-2.0"` reclassifies a package licensed exactly `MPL-2.0`, but not one licensed `MPL-2.0 AND MIT`; to reclassify that package you'd key the full expression (`"MPL-2.0 AND MIT"`). When a configured key matches no package, the report adds a warning, which catches both typos and keys that only appear as components of a larger expression.
